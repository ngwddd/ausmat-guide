...
`courses.js` 里是 **56 条课程事实**，来自原工作簿的课程数据库，每条只有事实：
...
`atar-vs-recorded-minimum` 在学生够得上某条已记录最低分时说话，并**点名两门课**：
够得上的最近一门，和够不上的最近一门，以及差多少分。够不上任何一门时，
`atar-below-all-recorded` 会说这件事——在这里保持沉默才是最坏的选择。
